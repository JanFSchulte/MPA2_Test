...
class MainTestsMPA():
    ...
    def Configure(self, directory = '../MPA2_Results/Wafer0/', runtest = 'default'):
        ...
        if(runtest == 'default'):
            ...
            self.runtest.set_enable('Calibrate Bias', 'OFF')
            self.runtest.set_enable('Calibrate VREF', 'OFF')
            self.runtest.set_enable('DACs', 'OFF')
            self.runtest.set_enable('S-Curve', 'OFF')
    ...
    def test_routine_strip_in(self, filename, runname = ''):
        ...
        while (en and wd < 3):
            try:
                strip_in = self.test.strip_in_scan(print_file = 1, filename = filename + "strip_input_scan", probe=0, verbose = 0)
                good_si = 1
                ...
                if good_si:
                    ...
                    strip_in = self.test.strip_in_scan(print_file = 1, edge = "rising", filename = filename + "strip_input_scan", probe=0, verbose = 0)
                    StripIn = 0
                    for i in range(0,8):
                        if (strip_in[i,:].all() > self.signal_integrity_limit): good_si = 1
                    if good_si:
