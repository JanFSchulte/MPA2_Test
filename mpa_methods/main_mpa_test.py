...
class main_mpa_test():
    ...
    def test_routine_parameters(self, filename = 'default', runname = '  0Mrad'):
        ...
        while self.runtest.is_active('Configuration') and wd < 3:
            try:
                self.ssa.save_configuration(self.dir_path + filename + '_Configuration_' + str(runname) + '.scv', display=False)
                break
            except:
    ...
    def idle_routine(self, filename = 'default', runname = ''):
        ...
        r1, r2, r3 = [0]*3
        filename = self.summary.get_file_name(self.dir_path + filename + '_IDLE')
        print(filename)
        try:
            time.sleep(0.1); r4, r5 = self.test.lateral_input_phase_tuning(display=False, file = self.dir_path + filename + '_lateral.csv', filemode = 'a', runname = runname + '_idle', shift = 0)
            time.sleep(0.1); r1 = self.test.cluster_data(mode = 'digital', nstrips = 3, shift = 0,  display=False, file = self.dir_path + filename + '_CL.csv', filemode = 'a', runname = runname + '_idle')
            time.sleep(0.1); r2, r3 = self.test.l1_data_basic(mode = 'digital', shift = 0, file = self.dir_path + filename + '_L1.csv', filemode = 'a', runname = runname + '_idle')
