			...
			if extract:
				try:
					if s_type == "THR":
						...
						par, cov = curve_fit(errorfc, list(range(start_DAC, (stop-start))), scurve[(r-1)*120+p,start_DAC + 1 :(stop-start) + 1], p0= [n_pulse, nominal_DAC, 2])
					elif s_type == "CAL":
						start_DAC = 0
						par, cov = curve_fit(errorf, list(range(start_DAC, (stop-start))), scurve[(r-1)*120+p,start_DAC + 1 :(stop-start) + 1], p0= [n_pulse, nominal_DAC, 2])
	...
	if plot:
		...
		if extract:
			...
			if len(th_array) == 1:
				...
				plt.hist(th_array[1:1920], bins=list(range(min(th_array), max(th_array) + 1, 1)))
			plt.figure(3)
...
#hit_map(100, 250, 200)
#OBSOLETE
def hit_map(n_pulse, cal, th, row = list(range(1,17)), pixel = list(range(2,120)) , print_file =0, filename = "../cernbox/MPA_Results/hitmap"):
	t0 = time.time()
	pixel = np.array(pixel)
	row = np.array(row)
	nrow = int(row.shape[0])
...
#OBSOLETE
def hit_map_plot(n_pulse = 1000, cal = 40, th = 100, row = list(range(1,17)), pixel = list(range(1,120))):
	clear_counters()
	t0 = time.time()
	pixel = np.array(pixel)
	row = np.array(row)
	...
	return data_array
#OBSOLETE
def heat_map(row = list(range(1,17)), start_data = 150, start_th = 160, end_th = 180, file_name = "Am-241_tin_long_cal_0.csv"):
...
def s_curve_rbr(n_pulse, cal, row, step = 1, start = 0, stop = 256, plot = 1, print_file =1, filename = "../cernbox/MPA_Results/scurve"):
	...
	if plot:
		for r in row:
			for p in range(1,120):
				plt.plot(list(range(0,nstep)), data_array[(r-1)*120+p,:],'-')
...
def s_curve_rbr_fr(n_pulse = 1000, s_type = "THR", ref_val = 50, row = list(range(1,17)), step = 1, start = 0, stop = 256, pulse_delay = 200, extract_val = 0, extract = 1, plot = 1, print_file =1, filename = "../cernbox/MPA_Results/scurve_fr_"):
	t0 = time.time()
	try:
		clear_counters(8)
	...
	if extract:
		...
		th_array, noise_array = plot_extract_scurve(row = row, pixel = list(range(1,120)), s_type = s_type, scurve = data_array , n_pulse = n_pulse, nominal_DAC = extract_val, start = start, stop = stop, plot = plot, extract = extract)
		t1 = time.time()
		print("END")
		print("Elapsed Time: " + str(t1 - t0))
		plt.show()
		...
		plot_extract_scurve(row = row, pixel = list(range(1,120)), s_type = s_type, scurve = data_array , n_pulse = n_pulse, nominal_DAC = extract_val, start = start, stop = stop, extract = 0, plot = plot)
	return data_array
...
def s_curve_pbp_fr(n_pulse = 1000, s_type = "THR", ref_val = 100, row = list(range(1,17)), pixel = list(range(1,120)), step = 1, start = 0, stop = 256, pulse_delay = 200,  extract_val = 110, extract = 0, plot = 1, print_file = 0, filename = "../cernbox/MPA_Results/scurve_fr_"):
	t0 = time.time()
	clear_counters(8)
	clear_counters(8)
	activate_I2C_chip(verbose = 0)
...
def s_curve_pbp_all(n_pulse = 1000, s_type = "CAL", ref_val = 100, row = list(range(1,17)), pixel = list(range(1,120)), step = 1, start = 0, stop = 256, pulse_delay = 200,  extract_val = 50, plot = 1, print_file = 0, filename = "../cernbox/MPA_Results/scurve_fr_"):
	t0 = time.time()
	clear_counters(8)
	clear_counters(8)
	activate_I2C_chip(verbose = 0)
...
def s_curve_pbp_test(n_pulse = 1000, s_type = "CAL", ref_val = 100, primary_row = list(range(1,17)), secondary_row = list(range(1,17)), primary_pixel = list(range(1,120)), secondary_pixel = list(range(1,120)), step = 1, start = 0, stop = 256, pulse_delay = 200,  extract_val = 50, plot = 1):
	t0 = time.time()
	clear_counters(8)
	clear_counters(8)
	activate_I2C_chip(verbose = 0)
	...
	if plot:
		for r in secondary_row:
			for p in secondary_pixel:
				...
				par, cov = curve_fit(errorfc, list(range(start_DAC, stop)), data_array[(r-1)*120+p,start_DAC + 1 :stop + 1], p0= [n_pulse, extract_val, 2])
				th_array[(r-1)*120+p] = int(round(par[1]))
				th_array = [a for a in th_array if a != 0]
				plt.plot(list(range(0,stop)), data_array[(r-1)*120+p,0:stop],'-')
				plt.show()
...
def trimming_noise(iteration = 1, nominal_DAC = 41, data_array = np.zeros(2040, dtype = np.int ), plot = 1, start = 0, stop = 150, ratio = 3.90, row = list(range(1,17)), pixel = list(range(1,120))):
	t0 = time.time()
	for r in row:
	...
	if plot:
		...
		for r in row:
			for p in pixel:
				plt.plot(list(range(start,stop+1)), scurve_init[(r-1)*120+p,:],'-')
		plt.xlabel('Threshold DAC value')
		plt.ylabel('Counter Value')
		plt.figure(2)
		for r in row:
			for p in pixel:
				plt.plot(list(range(start,stop+1)), scurve_final[(r-1)*120+p,:],'-')
		plt.xlabel('Threshold DAC value')
		plt.ylabel('Counter Value')
		plt.show()
	return data_array
...
def trimming_chip_noise(nominal_DAC = 75, nstep = 4, data_array = np.zeros(2040, dtype = np.int ), plot = 1, start = 0, stop = 150, ratio = 3.68, print_file = 0, filename = "../cernbox/MPA_Results/Test"):
	...
	for i in range(1,nstep+1):
		...
		data_array = trimming_noise(nominal_DAC = nominal_DAC, data_array = data_array, plot = 0, start = start, stop = stop, ratio = ratio, row = row, pixel = list(range(1,120)))
	for r in range(1,17):
		for p in range(1,120):
			I2C.pixel_write("TrimDAC",r,p,data_array[(r-1)*120+p])
	...
	if plot:
		...
		for r in range(0,17):
			for p in range(0,120):
				plt.plot(list(range(start,stop+1)), scurve[(r-1)*120+p,:],'-')
		plt.xlabel('Threshold DAC value')
		plt.ylabel('Counter Value')
		plt.show()
	if print_file:
...
def trimming_step(pix_out = [["Row", "Pixel", "DAC"]], n_pulse = 1000, s_type = "THR", ref_val = 10, iteration = 1, nominal_DAC = 110, data_array = np.zeros(2040, dtype = np.int ), plot = 1,  stop = 150, ratio = 3.90, row = list(range(1,17)), pixel = list(range(1,120))):
	t0 = time.time()
	for r in row:
		for p in pixel:
			I2C.pixel_write("TrimDAC",r,p,data_array[(r-1)*120+p])
		...
		for r in row:
			for p in pixel:
				try:
					if s_type == "THR":
						start_DAC = np.argmax(scurve[(r-1)*120+p,:]) + 10
						par, cov = curve_fit(errorfc, list(range(start_DAC, stop)), scurve[(r-1)*120+p,start_DAC + 1 :stop + 1], p0= [n_pulse, nominal_DAC, 2])
					elif s_type == "CAL":
						start_DAC = 0
						par, cov = curve_fit(errorf, list(range(start_DAC, stop)), scurve[(r-1)*120+p,start_DAC + 1 :stop + 1], p0= [n_pulse, nominal_DAC, 2])
...
def trimming_chip(s_type = "THR", ref_val = 10, nominal_DAC = 110, nstep = 4, data_array = np.zeros(2040, dtype = np.int ), n_pulse = 300, iteration = 2, extract = 1, plot = 1, stop = 256, ratio = 3.68, print_file = 0, filename = "../cernbox/MPA_Results/Test"):
	...
	if extract:
		scurve = s_curve_rbr_fr(n_pulse = n_pulse,  s_type = s_type, ref_val = ref_val, row = list(range(1,17)), step = 1, start = 0, stop = stop, pulse_delay = 200, extract = 0, plot = 0, print_file =0)
		th_array, noise_array = plot_extract_scurve(row = list(range(1,17)), pixel = list(range(1,120)), s_type = s_type, scurve = scurve , n_pulse = n_pulse, nominal_DAC = nominal_DAC, start = 0, stop = stop, plot = plot, extract = 1)
...
def trimDAC_linearity_rbr(row, pixel = list(range(1,120)), plot = 1, print_file = 0, filename = "../cernbox/MPA_Results/TrimDAC"):
	t0 = time.time()
	activate_I2C_chip(verbose = 0)
	...
	if plot:
		...
		for r in row:
			for p in pixel:
				plt.plot(list(range(0,32)), data[(r-1)*120+p,:],'o', label = (r-1)*120+p)
...
def char_DLL(row, pixel, delay_reset, curr = list(range(0,32)), plot = 1, print_file = 0, filename = "../cernbox/MPA_Results/DLL_char"):
	activate_pp()
	activate_sync()
	disable_pixel(0,0)
	...
	if plot:
		for i in curr:
			plt.plot(list(range(0,64)),latency[i],'o-', label = i)
...
def shaper_rise(row, pixel, delay_reset, BX, th = list(range(90,170,10)), cal = 50, iteration = 10, plot = 1, print_file = 1, filename = "../cernbox/MPA_Results/shaper_rise", sampling = "edge"):
	activate_I2C_chip(verbose = 0)
	activate_pp()
	activate_sync()
	disable_pixel(0,0)
...
def shaper_fall(row, pixel, delay_reset, BX, th = list(range(160,90,-10)), cal = 50, iteration = 10, plot = 1, print_file = 1, filename = "../cernbox/MPA_Results/shaper_fall", sampling = "edge"):
	activate_I2C_chip(verbose = 0)
	activate_pp()
	activate_sync()
	disable_pixel(0,0)
...
def timewalk(row, pixel, delay_reset, BX, cal = list(range(15,255,10)), th = 100, iteration = 10, plot = 1, print_file = 1, filename = "../cernbox/MPA_Results/timewalk"):
	activate_I2C_chip(verbose = 0)
	activate_pp()
	activate_sync()
	disable_pixel(0,0)
