...
class SSA_ProbeMeasurement():
	...
	def test_routine_dacs(self, filename = 'default', runname = ''):
		filename = self.summary.get_file_name(filename)
		time_init = time.time()
		wd = 0
		while self.runtest.is_active('Bias_THDAC') and wd < 3:
			try:
				r1, r2 = self.measure.dac_linearity(name = 'Bias_THDAC', eval_inl_dnl = False, nbits = 8, npoints = 10, filename = filename, plot = False, filemode = 'a', runname = runname)
				self.summary.set('Bias_THDAC_GAIN'    , r1*1E3, '', '',  runname)
				self.summary.set('Bias_THDAC_OFFS'    , r2*1E3, '', '',  runname)
				break
			except:
		...
		while self.runtest.is_active('Bias_CALDAC') and wd < 3:
			try:
				r1, r2 = self.measure.dac_linearity(name = 'Bias_CALDAC', eval_inl_dnl = False, nbits = 8, npoints = 10, filename = filename, plot = False, filemode = 'a', runname = runname)
				self.summary.set('Bias_THDAC_GAIN'    , r1*1E3, '', '',  runname)
				self.summary.set('Bias_THDAC_OFFS'    , r2*1E3, '', '',  runname)
				break
			except:
