...
class mpa_configurations():
    ...
    def convertNomToRawPixmap(self, data):
        if len(data) < self.npixsnom:
            print(
                "Input is not a nom pixel map for sure, too short list - return original list")
            return data
        rawdata = []
        for p in range(0, self.npixsraw):
    ...
    def getNomAndRawRowsCols(self, rows, cols, israw=False):
        ...
        if wasempty:
            ...
            for index in range(len(rawrows)):
                rawrows[index] = self.getrawrow(rows[index])
            for index in range(len(rawcols)):
                rawcols[index] = self.getrawcol(cols[index])
        else:
            for index in range(len(rows)):
                rows[index] = self.getnomrow(rows[index])
            for index in range(len(cols)):
                cols[index] = self.getnomcol(cols[index])
